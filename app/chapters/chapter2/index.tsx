...
export default function Chapter2Screen() {
	...
	const chapterProgress = getChapterProgress("chapter2");
	...
	// Unlock chapter 2 if not already unlocked
	React.useEffect(() => {
		if (!chapterProgress) {
	...
	const handleLessonPress = (type: string, lessonId?: string) => {
		...
		router.push(`/chapters/chapter2/${path}` as any);
	};
		...
		>
			...
			>
				...
				<View style={styles.section}>
					...
					>
						{chapter2Data.objectives.map((objective, index) => (
				...
				<View style={styles.section}>
					...
					<LessonCard
						title="Vocabulaire"
						subtitle="Apprenez les nombres, le temps et les expressions temporelles"
						type="vocabulary"
						completed={false}
						progress={0}
						onPress={() => handleLessonPress("vocabulary", "numbers")}
					/>
					<LessonCard
						title="Horaires"
						subtitle="Maîtrisez l'heure et les rendez-vous"
						type="vocabulary"
						completed={false}
						progress={0}
						onPress={() => handleLessonPress("vocabulary", "time")}
					/>
					<LessonCard
						title="Exercices"
						subtitle="Pratiquez les nombres et l'heure"
						type="exercises"
						completed={false}
						progress={0}
					...
					<LessonCard
						title="Jeux"
						subtitle="Jeux de timing et de planification"
						type="games"
						completed={false}
						progress={0}
						onPress={() => handleLessonPress("games", "timing")}
					/>
					<LessonCard
						title="Évaluation"
