...
export default function VocabularyLesson() {
	...
	const getVocabularyForLesson = () => {
		if (lessonId === "family") {
			return chapter3Data.vocabulary.filter(
				(word) => word.category === "family"
			);
		}
		if (lessonId === "descriptions") {
			return chapter3Data.vocabulary.filter(
				(word) =>
					word.category === "descriptions" || word.category === "appearance"
			);
		}
		if (lessonId === "places") {
			return chapter3Data.vocabulary.filter(
				(word) => word.category === "places" || word.category === "objects"
			);
		}
		return chapter3Data.vocabulary;
	};
	...
	const handleLessonComplete = async () => {
		...
		const xpAmount = score === 100 ? 40 : Math.max(20, Math.round(score * 0.3));
	...
	return (
		...
		>
			...
			<CelebrationModal
				...
				title="Vocabulaire Terminé! 📚"
				message={`Vous avez appris ${vocabularyList.length} nouveaux mots!`}
				type="lesson_complete"
				xpEarned={completedWords.size === vocabularyList.length ? 40 : 20}
			/>
		</SafeAreaView>
	);
}
